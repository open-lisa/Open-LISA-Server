...
def test_command_execution_result_should_be_init_with_CommandReturnType():
    with pytest.raises(Exception):
        CommandExecutionResult(
            timestamp_execution_begin=1, type="invalid type")


def test_command_execution_result_is_created_with_a_timestamps():
    now = get_UTC_timestamp()
    result = CommandExecutionResult(
        timestamp_execution_begin=now, type=CommandReturnType.VOID)
    assert isinstance(result.timestamp_execution_begin, float)
    assert result.timestamp_execution_begin == now
    assert isinstance(result.timestamp_execution_finish, float)


def test_command_execution_result_with_void_type():
    result = CommandExecutionResult(
        timestamp_execution_begin=1, type=CommandReturnType.VOID)
    assert result.value == None


def test_command_execution_result_with_string_type():
    result = CommandExecutionResult(
        timestamp_execution_begin=1,

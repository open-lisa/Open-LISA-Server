...
int pdc_get_device_name(UINT n_device_no, UINT n_child_no, const char * tmp_file_buffer) {
    ...
    pdc_get_device_name_function = (PDC_GET_DEVICE_NAME_FUNCTION_DLL) GetProcAddress(libHandle, "PDC_GetDeviceNameA");
    if (pdc_get_device_name_function == NULL) {
        const char * message = "GetProcAddress failed loading PDC_GetDeviceNameA function";
        fwrite(message, sizeof(char), strlen(message), output_file);
        fclose(output_file);
        return PDC_WRAPPER_FAILED;
    }
